...
function* handleBusinessesFetch(action: ReturnType<typeof fetchReviewsRequest>) {
  try {
    ...
    const res = yield call(
      ...
      "GET",
      API_ENDPOINT,
      `/businesses/search?location=los angeles&term=${action.payload}`
    );
...
function* handleReviewsFetch(action: ReturnType<typeof fetchReviewsRequest>) {
  try {
    const reviews = yield call(
      ...
      "GET",
      API_ENDPOINT,
      `/businesses/${action.payload}/reviews`
    );

    if (reviews.error) {
      yield put(fetchReviewsFailure(reviews));
    } else {
